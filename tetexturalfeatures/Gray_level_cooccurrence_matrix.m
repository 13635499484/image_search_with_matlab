%%灰度共生矩阵特征值计算
function [ b ] = Gray_level_cooccurrence_matrix( f )
glcms1=graycomatrix(f,'numlevels',64,'offset',[0 1;-1 1;-1 0;-1 -1]);  %计算64位灰度共生矩阵
stats=graycoprops(glcms1,{'contrast','correlation','energy','homogeneity'}); %纹理特征统计值(对比度、相关性、熵、平稳度、能量)
ga1=glcms1(:,:,1);
ga2=glcms1(:,:,2);
ga3=glcms1(:,:,3);
ga4=glcms1(:,:,4);
